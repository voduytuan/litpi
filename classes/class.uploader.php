...
class Uploader
{
	...
	function __construct($sourceTmpFile = '', $sourceName = '', $desitnationDirectory = '', $destinationName = '', $validType = null, $inValidType = null, $fileSizeInByte = 0)
	{
		...
		$this->maxFileSize = (int)($fileSizeInByte) > 0 ? $this->return_byte($fileSizeInByte) : $this->return_byte(ini_get('upload_max_filesize'));
	}
	
	
	function upload($overwrite = false, &$finalName = '')
	{
		$error = 0;
		//check file size
		...
		if($error == 0)
		{
			...
			$name=preg_replace("[^a-zA-Z0-9_.]","-", $newName);
	...
	function addDirSeparator($directory)
	{
