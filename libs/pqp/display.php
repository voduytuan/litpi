...
$cssUrl = $conf['rooturl'] . 'libs/pqp/css/pQp.css';
...
if($logCount ==  0) {
	...
	echo '<table class="side" cellspacing="0">
		...
		foreach($output['logs']['console'] as $log) {
			echo '<tr class="log-'.$log['type'].'" title="Called in: '.$log['callstack'].'">
...
if($output['queryTotals']['count'] ==  0) {
	...
	echo '<table class="side" cellspacing="0">
		...
		foreach($output['queries'] as $query) {
			echo '<tr>
				<td class="'.$class.'"><b>'.$query['time'].'</b>'.$query['sql'];
			if(!empty($query['params'])) {
					echo '<div style="padding:0 10px;"><br /><em>Params: </em>';
